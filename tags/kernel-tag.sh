...
# LTS
KERNEL_VERSION=`curl -s https://cdn.kernel.org/pub/linux/kernel/v6.x/sha256sums.asc | awk '{print $2}' | grep -E ^linux-6.13 | grep tar.xz | sed 's/linux-//g;s/.tar.xz//g' | tail -n 1`
